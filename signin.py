    ...
    def change_password():
        if user_entry.get()==''or newpass_entry.get()==''or confirmpass_entry=='':
            ...
            messagebox.showerror('Error','password and confirm password are not matching ',parent=window)
             ...
             con=pymysql.connect(host='localhost',user='root',password='2345',database='userdata')
             mycursor=con.cursor()
             ...
             if row==None:
                 ...
                 con.commit()
                 con.close()
                 messagebox.showinfo('Success','Password is reset successfully,please login with new password ',parent=window)
                 window.destroy()
...
def login_user():
    if usernameEntry.get()=='' or passwordEntry.get()=='':
        ...
        try:
            con=pymysql.connect(host='localhost',user='root',password='2345')
            mycursor=con.cursor()
        except:
...
heading=Label(login_window,text='USER LOGIN',font=('Microsofr Yahei UI Light',23,'bold')
                                           ,bg='white',fg='firebrick1')
heading.place(x=605,y=120)

usernameEntry=Entry(login_window,width=25,font=('Microsofr Yahei UI Light',11,'bold'),bd=0,fg='firebrick1')
usernameEntry.place(x=580,y=200)
usernameEntry.insert(0,'Username')

usernameEntry.bind('<FocusIn>',user_enter)

frame=Frame(login_window,width=250,height=2,bg='firebrick1')
frame.place(x=580,y=222)

passwordEntry=Entry(login_window,width=25,font=('Microsofr Yahei UI Light',11,'bold'),bd=0,fg='firebrick1')
